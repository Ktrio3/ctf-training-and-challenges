...
function sendMessageToChannel(channel, from, message) {
    ...
    if(typeof from !== 'string') {
        if(!clientManager.isSubscribedTo(from, channel)) {
            //console.log('Could not send message',msg,' from', clientManager.getUsername(from),'to',channel,'because he is not subscribed.')
            return false;
        }
    }
...
io.on('connection', function (client) {
    client.on('register', function(inUser) {
        try {
            ...
            newUser.country = "Hi there"
            console.log(newUser.country)
            console.log(newUser.__proto__.country)
    ...
    client.on('disconnect', function () {
        try {
            ...
            if(oldclient !== undefined) {
                for (const a of Object.entries(oldclient.ch)) {
                    return
                    if(!state) continue;
                    sendMessageToChannel(channel, "Server",
